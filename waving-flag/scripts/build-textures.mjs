...
const LONG_EDGE = data.textureLongEdge;
...
for (const c of data.countries) {
  ...
  // Longest edge gets LONG_EDGE pixels; the other follows the official ratio.
  const [w, h] =
    officialAspect >= 1
      ? [LONG_EDGE, Math.round(LONG_EDGE / officialAspect)]
      : [Math.round(LONG_EDGE * officialAspect), LONG_EDGE];
  ...
  await page.setContent(
    `<!doctype html><html><body style="margin:0;padding:0;background:#fff;overflow:hidden">${svg}</body></html>`,
    {waitUntil: 'load'},
  );
  const buf = await page.screenshot({type: 'png', clip: {x: 0, y: 0, width: w, height: h}});
  writeFileSync(join(OUT_DIR, `${c.code}.png`), buf);
  await page.close();

  report.push(
    `ok   ${c.code} ${c.name.padEnd(22)} ${String(w).padStart(4)}x${String(h).padStart(4)}  ` +
      `${c.ratio[0]}:${c.ratio[1]}  ${(buf.length / 1024).toFixed(0)} KiB` +
...
console.log(`\n${data.countries.length} textures written to public/flags at ${LONG_EDGE}px long edge.`);
