...
export const paintBadge = (
  ...
  fill.addColorStop(0, mixColors(theme.cardBlue, theme.backgroundMid, 0.25));
  fill.addColorStop(0.55, theme.cardBlue);
  fill.addColorStop(1, mixColors(theme.cardBlue, theme.glowCyan, 0.8));
  ctx.fillStyle = fill;
  ...
  ctx.strokeStyle = theme.glowCyan;
  ...
  (ctx as CanvasRenderingContext2D & {letterSpacing?: string}).letterSpacing = `${-fontSize * 0.03}px`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'alphabetic';

  const metrics = ctx.measureText(CONFIG.hero.glyph);
  const centreOffset =
    (metrics.actualBoundingBoxAscent - metrics.actualBoundingBoxDescent) / 2;

  ctx.fillStyle = theme.badgeWhite;
  ctx.fillText(CONFIG.hero.glyph, 0, centreOffset);
