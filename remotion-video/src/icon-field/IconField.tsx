...
export const iconFieldSchema = z.object({
  variant: z.enum(["blue", "teal", "breach"]),
});

export type IconFieldProps = z.infer<typeof iconFieldSchema>;
 ...
 */
export const IconField: React.FC<IconFieldProps> = ({ variant }) => {
  const frame = useCurrentFrame();
  const cfg = VARIANTS[variant as VariantKey];
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const env = useMemo(() => createEnv(variant, cfg), [variant, cfg]);
  const fs = useMemo(
