/**
 * 4D simplex noise (Ashima Arts / Stefan Gustavson, MIT licensed).
 *
 * Four dimensions matter here: the first two carry screen space, and the last
 * two trace a circle over the clip's duration. Sampling a 4D field along a
 * closed circular path makes the resulting 2D animation exactly periodic, which
 * is what gives every composition a seamless loop with no crossfade.
 */
...
float permute1(float x) { return mod289f(((x * 34.0) + 1.0) * x); }
