// A tiny deterministic PRNG (mulberry32). Any "identity" value (angle,
// radius, size, phase...) is derived from an index through this, never
...
// (index, frame) would cause popping/flicker between frames.
...
export const seededInt = (
  ...
  min: number,
  max: number,
) => Math.floor(seededRange(index, salt, min, max + 1 - 1e-9));
