// Proves the composition loops: renders frame 0 and frame DURATION_IN_FRAMES
...
const probe = { ...composition, durationInFrames: composition.durationInFrames + 1 };
const outputs = [];

for (const frame of [0, composition.durationInFrames]) {
  const output = `out/loop-${frame}.png`;
  await renderStill({
    composition: probe,
    serveUrl,
    output,
...
console.log(
  `frame 0 vs frame ${composition.durationInFrames} @ ${composition.width}x${composition.height}: ` +
