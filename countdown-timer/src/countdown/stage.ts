/**
 * The single drawing surface the four components share.
 *
 * Everything is painted onto ONE visible canvas, additively over pure
 * black, so component order does not matter and the layers cannot
 * accidentally occlude one another. Each component paints its artwork
 * into `scratch`, blooms it, then composites the result into `target`.
 *
 ...
 * component can double-expose itself if React happens to re-run its
 * effect without the frame having advanced.
 */
export type Stage = {
  width: number;
  height: number;
