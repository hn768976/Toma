...
export interface BokehLayerProps {
  ...
  /** The two hues discs are mixed between. */
  colorA: string;
  colorB: string;
  frame: number;
  loop: number;
  ...
  useCanvasLayer({
    ...
    draw: (ctx) => {
      ...
      for (let i = from; i < to && i < discs.length; i++) {
        ...
        g.addColorStop(0, mixRgba(colorA, colorB, b.tone, a * 0.5));
        g.addColorStop(0.55, mixRgba(colorA, colorB, b.tone, a * 0.34));
        // A faint rim: real bokeh is brighter at the edge of the disc.
        g.addColorStop(0.88, mixRgba(colorA, colorB, b.tone, a * 0.46 * rim));
        g.addColorStop(1, mixRgba(colorA, colorB, b.tone, 0));
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.arc(x, y, r, 0, TAU);
        ctx.fill();
