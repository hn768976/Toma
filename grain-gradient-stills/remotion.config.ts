/**
 * Note: when using the Node APIs the config file does not apply; pass the
 * same options directly to the APIs instead.
 */
...
// The colour field is computed at full resolution on purpose, so a still
// takes a while to draw. The default timeout is not generous enough at 4K.
